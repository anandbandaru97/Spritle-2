...
const AgentsList = ({ trigger }) => {
  ...
  useEffect(() => {
    console.log("test");
    setList(filteredData); //eslint-disable-next-line
  }, [trigger]);
  ...
  return (
    <>
      ...
      <div>
        {list &&
          list.map((user) => (
            <div className="main" key={v4()}>
              <div className="card">
                <h3 key={v4()}>Email: {user.Email} </h3>
                <h3>Password: {user.Password}</h3>{" "}
              </div>{" "}
              <div>
                <button onClick={onClickHandler} id={user.Username}>
                  {" "}
                  X{" "}
                </button>
              </div>
